...
from MediaCrawler import cmd_arg
from MediaCrawler import config
from MediaCrawler.database import db
...
from MediaCrawler.media_platform.tieba import TieBaCrawler
from MediaCrawler.media_platform.weibo import WeiboCrawler
from MediaCrawler.media_platform.xhs import XiaoHongShuCrawler
from MediaCrawler.media_platform.zhihu import ZhihuCrawler
from MediaCrawler.tools.async_file_writer import AsyncFileWriter
...
class CrawlerFactory:
    ...
    @staticmethod
    def create_crawler(platform: str, keywords: str = None) -> AbstractCrawler:
        ...
        return crawler_class(keywords=keywords)
...
def _flush_excel_if_needed() -> None:
    ...
    try:
        from MediaCrawler.store.excel_store_base import ExcelStoreBase
...
async def main(keywords: str = None) -> None:
    global crawler
    ...
    crawler = CrawlerFactory.create_crawler(platform=config.PLATFORM, keywords=keywords)
    await crawler.start()
...
if __name__ == "__main__":
    from MediaCrawler.tools.app_runner import run
